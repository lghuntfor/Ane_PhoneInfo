	...
	public FREObject call(FREContext pContext, FREObject[] params) {
		...
		try {
			...
			if(params.length>1){
				...
				Method method = PhoneInfo.class.getMethod(methodName, classes);
				value = method.invoke(phoneInfo,args).toString();
			} else {
				Method method = PhoneInfo.class.getMethod(methodName);
				value = method.invoke(phoneInfo).toString();
			}
